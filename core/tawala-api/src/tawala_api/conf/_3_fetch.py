...
class _FetchField:
    ...
    def _get_from_toml(self) -> Any:
        ...
        if self.toml is None:
            return None
        try:
            current: Any = PROJECT_API.base_toml
    ...
    def _fetch_value(self) -> Any:
        ...
        if self.env is not None:
            try:
                if self.env in PROJECT_API.base_env:
                    return PROJECT_API.base_env[self.env]
            except FetchProjectValidationError:
...
class _FetchLayout:
    ...
    option = _build_fetch_field(LAYOUT_SCHEMA, LayoutKeys.OPTION)
    dashboard = _build_fetch_field(LAYOUT_SCHEMA, LayoutKeys.DASHBOARD)


LAYOUT_API = _FetchLayout()
...
class _FetchDatabases:
    ...
    _is_vercel_preset = PRESET_API.option == PresetOptions.VERCEL
...
class _FetchSecurity:
    ...
    always_show_admin = _build_fetch_field(
        SECURITY_SCHEMA, SecurityKeys.ALWAYS_SHOW_ADMIN_OPTION
    )


SECURITY_API = _FetchSecurity()
...
INTERNATIONALIZATION_API = _FetchInternationalization()
...
RUNCOMMANDS_API = _FetchRuncommands()
