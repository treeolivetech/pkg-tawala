...
__all__ = [
    ...
    "SecurityAlwaysShowAdminOptions",
    "SecurityDefaults",
    "SecurityHelpTexts",
...
class LayoutDefaults(StrEnum):
    """Default values for layout configuration."""

    OPTION = LayoutOptions.UI.value
    DASHBOARD = "app"
...
class LayoutHelpTexts(StrEnum):
    ...
    OPTION = (
        "Layout option used by the app. "
        f"Defaults to the standard {LayoutDefaults.OPTION.value} layout."
    )
    DASHBOARD = (
        f"Dashboard app name. Defaults to the '{LayoutDefaults.DASHBOARD.value}'."
    )
...
class PresetHelpTexts(StrEnum):
    ...
    OPTION = "Preset that controls opinionated defaults for the project."
    BLOB_TOKEN = "Token used for blob storage read/write access."
...
class DatabaseHelpTexts(StrEnum):
    ...
    USE_VARS_OPTION = (
        "Enable reading PostgreSQL connection values from individual DB_PG* .env variables. "
    ...
    SERVICE = "PostgreSQL service name from pg_service file."
...
# always show admin
class SecurityAlwaysShowAdminOptions(Enum):
...
class SecurityDefaults(Enum):
    ...
    ALWAYS_SHOW_ADMIN_OPTION = SecurityAlwaysShowAdminOptions.DISABLED.value
