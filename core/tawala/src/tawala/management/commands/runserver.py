...
class Command(RunserverCommand):
    ...
    help = f"{settings.BASE_DISPLAY_NAME} Development server"
    ...
    def check_migrations(self) -> None:
        ...
        self.stdout.write(
            self.style.NOTICE(f"Run {settings.BASE_NAME} migrate to apply them.")
        )
    ...
    def _print_version(self) -> None:
        """Print version."""
        self.stdout.write(
            f"  🔧 {settings.BASE_DISPLAY_NAME} version: {self.style.HTTP_NOT_MODIFIED(settings.BASE_VERSION)}"
        )
