...
class Command(RunserverCommand):
    ...
    help = f"{settings.CORE_DISPLAY_NAME} Development server"
    ...
    def check_migrations(self) -> None:
        ...
        self.stdout.write(
            self.style.NOTICE(f"Run {settings.CORE_NAME} migrate to apply them.")
        )
    ...
    def _print_version(self) -> None:
        """Print version."""
        self.stdout.write(
            f"  🔧 {settings.CORE_DISPLAY_NAME} version: {self.style.HTTP_NOT_MODIFIED(settings.CORE_VERSION)}"
        )
