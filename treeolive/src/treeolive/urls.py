...
from treeolive_api.conf import LayoutOptions
...
match settings.LAYOUT:
    case LayoutOptions.WIP.value:
        urlpatterns = [path("", include(f"{settings.LAYOUT_APPS}.wip.urls"))]
    case _:
        urlpatterns = [path("", include(f"{settings.MAIN_APP}.urls"))]

urlpatterns += [
    *(
        [path("widgets/logout/", include(f"{settings.WIDGET_APPS}.logout.urls"))]
        if apps.is_installed(f"{settings.WIDGET_APPS}.logout")
        else []
    ),
    *(
