...
MANAGEMENT_CONF: Final = _ManagementConf()
...
class ConfField:
    ...
    def _get_from_toml(self) -> Any:
        ...
        if self.toml is None:
            return None
        current: Any = MANAGEMENT_CONF.toml
    ...
    def _fetch_value(self) -> Any:
        ...
        if self.env is not None and self.env in MANAGEMENT_CONF.env:
            return MANAGEMENT_CONF.env[self.env]
...
class SettingsConf:
    ...
    _subclasses: list[type["SettingsConf"]] = []

    def __init_subclass__(cls) -> None:
        """Register subclass and collect env field metadata."""
        super().__init_subclass__()
        SettingsConf._subclasses.append(cls)  # Register this subclass
